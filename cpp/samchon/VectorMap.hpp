...
namespace samchon
{
	...
	//PRE-TYPEDEFS
	template<typename T> using VectorDictionary = VectorMap<String, T>;
	...
	public:
		...
		VectorMap(vector&& x) : super(std::move(x)) { reIndex(); };
		VectorMap(vector&& x, const allocator_type& alloc) : super(std::move(x), alloc) { reIndex(); };
