...
public:
	...
	void run()
	{
		new Event(this, Event::ACTIVATE);
		dispatch(shared_ptr<Event>(new Event(this, Event::ACTIVATE)));

		for (size_t i = 0; i < 100; i++)
		{
			this_thread::sleep_for(chrono::seconds(1));
			dispatch(shared_ptr<Event>(new ProgressEvent(this, i + 1, 100)));
		}

		dispatch(shared_ptr<Event>(new Event(this, Event::COMPLETE)));
	};
};

void handleActivate(shared_ptr<Event> event, void *addiction)
{
	cout << "Activated" << endl;
}
void handleComplete(shared_ptr<Event> event, void *addiction)
{
	cout << "Completed" << endl;
}
void handleProgress(shared_ptr<Event> event, void *addiction)
{
