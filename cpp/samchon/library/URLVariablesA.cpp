...
URLVariablesA::BasicURLVariables(const WeakStringA &flashVars)
	: super()
{
	vector<WeakStringA> &parameterArray = flashVars.split("&");
	for (size_t i = 0; i < parameterArray.size(); i++)
	{
		WeakStringA &parameter = parameterArray[i];
		size_t index = parameter.find("=");
		if (index == string::npos)
			continue;

		WeakStringA &name = parameter.substr(0, index);
		WeakStringA &value = parameter.substr(index + 1);
...
template<> auto URLVariablesA::encode(const WeakStringA &wStr) -> string
{
