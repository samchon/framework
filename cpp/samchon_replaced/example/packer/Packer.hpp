		...
		namespace packer
		{
			...
			public:
				...
				Packer()
					: super()
				{
					this->productArray = std::make_shared<ProductArray>();
				};
				 ...
				 */
				Packer(std::shared_ptr<ProductArray> productArray)
					: super()
				{
			...
			public:
				...
				void optimize(size_t start = 0, size_t size = -1)
				{
					...
					library::CombinedPermutationGenerator caseGenerator(this->size(), productArray->size());
					std::shared_ptr<Packer> minPacker = nullptr;
					...
					{
						std::vector<size_t> &row = caseGenerator[i];
						std::shared_ptr<Packer> packer(new Packer(*this));

						bool validity = true;

						for (size_t j = 0; j < row.size(); j++)
						{
							std::shared_ptr<Product> &product = productArray->at(j);
							std::shared_ptr<WrapperArray> &wrapperArray = packer->at( row[j] );
				...
				{
					std::string str = "$" + std::to_string(calcPrice()) + "\n";
					for (size_t i = 0; i < size(); i++)
						str += at(i)->toString() + "\n";
				...
				{
					// CONSTRUCT PRODUCTS
					std::shared_ptr<ProductArray> productArray(new ProductArray());
