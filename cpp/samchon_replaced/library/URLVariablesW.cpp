...
URLVariablesW::BasicURLVariables(const WWeakString &flashVars)
	: super()
{
	vector<WWeakString> &parameterArray = flashVars.split(L"&");
	for (size_t i = 0; i < parameterArray.size(); i++)
	{
		WWeakString &parameter = parameterArray[i];
		size_t index = parameter.find(L"=");
		if (index == wstring::npos)
			continue;

		WWeakString &name = parameter.substr(0, index);
		WWeakString &value = parameter.substr(index + 1);
...
------------------------------------------------------------------------ */
template<> auto URLVariablesW::encode(const WWeakString &weakString) -> wstring
{
