...
``` typescript
class DerivedClass extends Something implements library.IEventDispatcher
{
...
{
	let list_collection: collection.ListCollection<string>();
	list_collection.addEventListener(collection.CollectionEvent.INSERT, handle_change);
	list_collection.addEventListener(collection.CollectionEvent.ERASE, handle_change);
	list_collection.addEventListener(collection.CollectionEvent.REFRESH, handle_change);
	
	list_collection.push_back("Jeongho Nam");
	list_collection.push_back("John Doe");
	list_collection.push_back("Micle");
	list_collection.insert(list_collection.end(), 100, "Unknown: #" + Math.random());
	
	list_collection.erase(list_collection.begin(), list_collection.begin().advance(5));
	list_collection.erase(list_collection.begin(), list_collection.begin().advance(10));
	list_collection.clear();
	
	list_collection.insert("Samchon Library");
	
	list_collection.begin().value = "Samchon Framework";
	list_collection.refresh();
}

function handle_change(event: collection.CollectionEvent)
{
	if (event.type == collection.CollectionEvent.REFRESH)
	{
		console.log("Refreshed");
		return;
	}
	
	console.log("Elements I/O has detected: typeof " + event.type); // insert or erase
	console.log("Elements affected the operations are: ");
