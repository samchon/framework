...
namespace samchon
{
	...
	//PRE-TYPEDEFS
	template<typename C, typename T> using BasicVectorDict = VectorMap<basic_string<C>, T>;
	...
	public:
		...
		//VectorMap(initializer_list<T> il, const allocator_type& alloc = allocator_type()) : vector<shared_ptr<T>>(il, alloc) { reIndex(); };
