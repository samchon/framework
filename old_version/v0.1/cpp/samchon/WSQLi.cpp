...
namespace samchon
{
	...
	template<> void WSQLi::connect(const wstring &ip, const wstring &db, const wstring &id, const wstring &pwd)
	{
		SQLRETURN res;
			...
			if ((res = SQLSetEnvAttr(environment, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0)) == SQL_SUCCESS)
				if ((res = SQLAllocHandle(SQL_HANDLE_DBC, environment, &hdbc)) == SQL_SUCCESS)
					res =
						...
						(
							hdbc, NULL,
							(SQLWCHAR *)&WStringUtil::substitute
							(
								L"DRIVER={0};SERVER={1}, {2};DATABASE={3};UID={4};PWD={5};",
	...
	template<> void WSQLi::disconnect()
	{
		if (stmt != nullptr)
			stmt->free();
	...
	template<> auto WSQLi::getErrorMessage(short type) const -> string
	{
		char state[1024];
		char message[1024];
